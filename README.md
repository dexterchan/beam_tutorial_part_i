...
Run command:
```
java -classpath build/libs/beamPiDemo-0.1-all.jar io.exp.apachebeam.text.BeamPiRun --inputFile=./config/test/instruction.dat --output=/tmp/PiTest
```
...
Run command:
```
java -classpath build/libs/beamPiDemo-0.1-all.jar io.exp.apachebeam.pubsub.BeamPiRun --inputTopic=projects/peer2peer/subscriptions/pi_in_sub --outputTopic=projects/peer2peer/topics/pi_out 
```
Pubsub injector:
````
java -classpath build/libs/beamPiDemo-0.1-all.jar io.exp.pubsubinjector.PiPubSubInjector <project> <topic> <number of msg>
java -classpath build/libs/beamPiDemo-0.1-all.jar io.exp.pubsubinjector.PiPubSubInjector peer2peer pi_in 1
...
```
Local gradle build
java -classpath build/libs/beamPiDemo-0.1-all.jar io.exp.apachebeam.text.BeamPiRun \
...
--output=/tmp/PiFlink \
--filesToStage=build/libs/beamPiDemo-0.1-all.jar \
--parallelism=4 \
--maxBundleSize=200
...
```
Local gradle build:
java -classpath build/libs/beamPiDemo-0.1-all.jar io.exp.apachebeam.kafka.BeamPiRun \
--runner=FlinkRunner \
--flinkMaster=localhost:9081 \
--bootStrapServer=localhost:9092 \
--inputTopic=pi \
--outputTopic=pi_out \
--filesToStage=/Users/dexter/sandbox/apachebeam/beam_tutorial_part_i/build/libs/beamPiDemo-0.1-all.jar \
--parallelism=2
...
java -classpath build/libs/beamPiDemo-0.1-all.jar io.exp.apachebeam.pubsub.BeamPiRun \
--runner=FlinkRunner \
--flinkMaster=localhost:9081 \
...
````
gradle -Pflink clean build
export JOB_JAR_TARGET=build/libs/beamPiDemo-0.1-all.jar
...
Run command:
```
java -classpath beamPiDemo-0.1-all.jar io.exp.apachebeam.inmemory.BeamPiRun
```